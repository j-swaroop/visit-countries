...
class VisitCountries extends Component {
  state = {
    initialCountriesList: countriesList,
  }

  removeCountry = id => {
    this.setState(prevState => ({
      initialCountriesList: prevState.initialCountriesList.map(item => {
        if (item.id === id) {
  ...
  render() {
    ...
    const visitedCountriesLength = visitedCountries.length !== 0
        ...
        <div className="responsive-container">
          ...
          {visitedCountriesLength ? (
            ...
            <div className="no-visited-countries-container">
              <p className="no-contries-text"> No Countries Visited Yet!</p>
            </div>
          )}
        </div>
      </div>
